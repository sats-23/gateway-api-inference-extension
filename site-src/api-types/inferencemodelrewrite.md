...
*   **Model Aliasing**: Map a model name in the request body (e.g., `qwen-uncensored`) to a specific version (e.g., `qwen-uncensored-v1`).
...
Map a virtual model name (e.g., `qwen-uncensored`) to a specific backend model version (e.g., `qwen-uncensored-v1`).

```yaml
apiVersion: inference.networking.x-k8s.io/v1alpha2
kind: InferenceModelRewrite
metadata:
  name: qwen-uncensored-alias
spec:
  poolRef:
